...
const alertTypeToSummaryFormatter: Record<string, (data: any) => string> = {
    FORM_SUBMITTED: (data) => `${data.name} submitted`,
    APPOINTMENT_SCHEDULED: (data) => `${data.title} scheduled for ${formatDate(data.start)}`,
    MESSAGE_RECEIVED: (data) => data.message,
...
export const getPatientTimeline = (): TimelineEntry[] => {
    ...
    const timelineEntries: TimelineEntry[] = [
        ...
        ...alerts.map((alert) => ({
            id: alert.id,
            type: "alert",
            icon: alertTypeToIcon[alert.type] || Bell,
            title: alertTypeToTitle[alert.type] || alert.type,
            date: formatDate(alert.createdDate),
            summary: (alertTypeToSummaryFormatter[alert.type] || alertTypeToSummaryFormatter.DEFAULT)(alert.data),
            category: "Alerts",
            actionLabel: typeof timelineTypeToActionLabel.alert === 'function'
                ? (timelineTypeToActionLabel.alert as (type: string) => string)(alert.type)
                : undefined,
        })),
